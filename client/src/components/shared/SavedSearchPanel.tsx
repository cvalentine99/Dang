  ...
  return (
    <>
      ...
      <div className="relative">
        ...
        {showDropdown && (
          <div className="absolute right-0 top-full mt-1 w-80 bg-[oklch(0.15_0.005_260)] border border-white/10 rounded-lg shadow-xl z-50 max-h-[400px] overflow-y-auto">
            <div className="p-3 border-b border-white/10">
              <h4 className="text-xs font-semibold text-amber-300">Saved {label} Searches</h4>
            </div>
      ...
      {showSaveDialog && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => setShowSaveDialog(false)}>
          <div className="bg-[oklch(0.15_0.005_260)] border border-white/10 rounded-xl p-6 w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-500/50"
                  autoFocus
                />
              </div>
              <div>
              ...
              {filterSummary && filterSummary.length > 0 && (
                <div className="bg-white/5 rounded-lg p-3 text-xs text-slate-400 space-y-1">
                  ...
                  {filterSummary.map((f) => (
                    <p key={f.label}>{f.label}: <span className="font-mono text-amber-300">{f.value}</span></p>
                  ))}
                </div>
              )}
            </div>
