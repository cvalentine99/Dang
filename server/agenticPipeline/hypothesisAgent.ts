...
export interface HypothesisAgentInput {
  ...
  /** Optional: authenticated user ID to attribute the session to */
  userId?: number;
}
...
async function resolveSession(
  ...
  if (caseRec?.action === "merge_existing" && caseRec.mergeTargetId) {
    const [target] = await db
      ...
      .from(investigationSessions)
      .where(and(
        eq(investigationSessions.id, caseRec.mergeTargetId),
  ...
  const [result] = await db.insert(investigationSessions).values({
    ...
    status: "active",
    // Use spread to conditionally include userId — avoids Drizzle null-serialization bug
    // When userId is provided (e.g., from Continue Pipeline), attribute session to the analyst
    // When omitted, column defaults to NULL for system-created sessions
    ...(userId ? { userId } : {}),
  }).$returningId();
...
function assembleLivingCase(
  ...
  return {
    ...
    // DEPRECATED: recommendedActions[].state is set to "proposed" here and never
    // synced. The canonical source of truth for action states is the response_actions
    // table. Consumers should read actionSummary or query response_actions directly.
    // See: livingCaseReportService.ts which correctly reads from the table.
    recommendedActions: (llmOutput.recommendedActions ?? []).map((a) => ({
      action: a.action ?? "",
      category: normCategory(a.category ?? "next"),
      // Urgency uses canonical DB enum: immediate|next|scheduled|optional
      urgency: (() => {
        const VALID: string[] = ["immediate", "next", "scheduled", "optional"];
        const FALLBACK: Record<string, string> = {
          high: "immediate", medium: "next", low: "optional",
        };
        const raw = a.urgency?.toLowerCase() ?? "";
        type Urgency = "immediate" | "next" | "scheduled" | "optional";
        if (VALID.includes(raw)) return raw as Urgency;
        return (FALLBACK[raw] ?? "next") as Urgency;
      })(),
...
function mergeLivingCases(
  ...
  return {
    ...
    // Keep all existing actions and append new ones (dedup by action text).
    // Previous filter removed all "proposed" actions, but state is always "proposed"
    // in the snapshot (canonical state lives in response_actions table).
    recommendedActions: [
      ...existing.recommendedActions,
      ...incoming.recommendedActions.filter((inc) =>
        !existing.recommendedActions.some((ex) => ex.action === inc.action)
      ),
    ],
...
export async function runHypothesisAgent(
  ...
  const { sessionId, isNew } = await resolveSession(ctx, input.existingSessionId, input.userId);
  ...
  if (materializedActionIds.length > 0) {
    ...
    if (dbUpdate) {
      await dbUpdate.transaction(async (tx) => {
        ...
        const freshSummary = await recomputeCaseSummary(caseStateId, tx);
        if (freshSummary) {
...
// ── Response-Actions-Only Recovery ──────────────────────────────────────────

/**
 * Re-materialize response actions for an existing living case without re-running
 * the hypothesis LLM. Used by resume/continue when only response actions
 * failed or partially failed — avoids duplicating sessions/cases.
 */
export async function rematerializeResponseActions(input: {
  livingCaseId: number;
  correlationId: string;
}): Promise<{
  actionIds: string[];
  partialFailure: HypothesisAgentResult["materializePartialFailure"];
  latencyMs: number;
}> {
  const startTime = Date.now();
  const db = await getDb();
  ...
  // Load the existing living case
  const [caseRow] = await db
    .select()
    .from(livingCaseState)
    .where(eq(livingCaseState.id, input.livingCaseId))
    .limit(1);
  if (!caseRow) throw new Error(`Living case ${input.livingCaseId} not found`);

  const existingCase = caseRow.caseData as LivingCaseObject;
  if (!existingCase?.recommendedActions?.length) {
    return { actionIds: [], partialFailure: null, latencyMs: Date.now() - startTime };
  }

  // Load the context for provenance
  const ctx = await assembleContext(input.correlationId);

  // Materialize — only creates rows for actions not already materialized
  const result = await materializeResponseActions(existingCase, input.livingCaseId, ctx);
  const actionIds = result.ids;

  // Post-materialization sync (same as in runHypothesisAgent)
  if (actionIds.length > 0) {
    await db.transaction(async (tx) => {
      ...
      const persistedCase = (persistedRow?.caseData as LivingCaseObject) ?? existingCase;
      const existing = persistedCase.recommendedActionIds ?? [];
      persistedCase.recommendedActionIds = Array.from(new Set([...existing, ...actionIds]));

      const freshSummary = await recomputeCaseSummary(input.livingCaseId, tx);
      if (freshSummary) persistedCase.actionSummary = freshSummary;
    ...
    await syncCaseSummaryAfterTransition(input.livingCaseId);
  }
...
export async function getLivingCaseByCorrelationId(correlationId: string) {
  ...
  // SQL-native JSON search — no in-memory scan, no limit(50) ceiling
  const [row] = await db
    .select()
    .from(livingCaseState)
    .where(sql`JSON_CONTAINS(${livingCaseState.linkedCorrelationIds}, ${JSON.stringify(correlationId)})`)
    .limit(1);
...
async function materializeResponseActions(
  ...
  // ── BUG-01 Fix: Dedup against existing proposed actions for this case ──────
  // Prevents re-inserting the same action on resume or if the input list has duplicates.
  // Dedup key: normalized title + category (scoped to proposed state for this caseId).
  const existingRows = await db
    .select({ title: responseActions.title, category: responseActions.category })
    .from(responseActions)
    .where(and(
      eq(responseActions.caseId, caseId),
      eq(responseActions.state, "proposed"),
    ));
  const dedupSet = new Set<string>(
    existingRows.map(r => `${r.title.trim().toLowerCase()}||${r.category}`)
  );
  ...
  // LivingCaseObject now uses the same enum as DB — direct passthrough with fallback
  const URGENCY_FALLBACK: Record<string, string> = {
    high: "immediate",
    medium: "next",
    low: "optional",
  };
  ...
  for (const rec of actions) {
    try {
      ...
      // ── BUG-01 Fix: Skip duplicate actions (dedup by title+category in proposed state) ──
      const normalizedTitle = (rec.action ?? "Unnamed action").slice(0, 512).trim().toLowerCase();
      const dedupKey = `${normalizedTitle}||${category}`;
      if (dedupSet.has(dedupKey)) {
        continue; // Already materialized — skip to prevent duplicate row
      }

      const actionId = `ra-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;

      // Resolve urgency — direct passthrough since LivingCaseObject now uses DB enum
      const recUrgency = rec.urgency?.toLowerCase() ?? "";
      let urgency = VALID_URGENCY.includes(recUrgency)
        ? recUrgency
        : URGENCY_FALLBACK[recUrgency] ?? "next";
      ...
      // BUG-01: Update in-memory dedup set so later items in the same loop are skipped
