...
describe("Backend resumePipelineRun — Partial Run Continuation", () => {
  ...
  describe("Replay run record for partial continuation", () => {
    it("carries forward triageId when starting from correlation (spread pattern)", () => {
      // After null-serialization fix: uses spread to conditionally include triageId
      // instead of ternary with null (which Drizzle serializes as empty string)
      expect(src).toContain("triageId: originalRun.triageId,");
      expect(src).toContain("startIdx > 0 ?");
    });
