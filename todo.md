...
- [x] Push current codebase to cvalentine99/Dang- repository (commit d614c61)
- [x] Create GitHub release tagged as v1.0.0-rc1 "Dang! RC1"

## Broker Coverage Fix — Wire ALL 121 Endpoints to 100% (2026-03-10)
- [x] Audit current broker configs — started at 54/127 broker-wired (43%)
- [x] Wire missing Manager endpoints (stats, daemons, version check, component config, info, status, etc.)
- [x] Wire missing Cluster endpoints (healthcheck, ruleset sync, api config, node info/stats/status/logs, etc.)
- [x] Wire missing Agent endpoints (summary, by-id, daemon stats, config, upgrade result, group sync, etc.)
- [x] Wire missing Syscollector endpoints (os, hardware, browser extensions, users, groups)
- [x] Wire missing Experimental endpoints (already broker-wired)
- [x] Wire missing Rules endpoints (groups, requirement, file content)
- [x] Wire missing MITRE endpoints (metadata)
- [x] Wire missing SCA endpoints (already broker-wired)
- [x] Wire missing CIS-CAT endpoints (already broker-wired)
- [x] Wire missing FIM/Syscheck endpoints (last_scan)
- [x] Wire missing Rootcheck endpoints (last_scan)
- [x] Wire missing Decoders endpoints (file content)
- [x] Wire missing Tasks endpoints (already broker-wired)
- [x] Wire missing Security endpoints (user/role/policy/rule by-id, resources, current user policies)
- [x] Wire missing Lists endpoints (file content)
- [x] Wire missing Groups endpoints (configuration, file content)
- [x] Run tests — 107 passed, 1 flaky timeout (stateMachine passes in isolation), 3155 total tests
- [x] Final: 121/121 = 100% broker-wired, 0 manual, 0 passthrough
- [x] Push hotfix to GitHub hotfix/rc1 branch (commit 6899f56)

## Investigation Sessions Insert Bug (2026-03-11)
